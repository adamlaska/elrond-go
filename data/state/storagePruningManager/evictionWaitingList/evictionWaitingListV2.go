...
func (ewl *evictionWaitingListV2) Put(rootHash []byte, hashes data.ModifiedHashes) error {
	ewl.opMutex.Lock()
...
func (ewl *evictionWaitingListV2) putInInversedCache(rootHash []byte, hashes data.ModifiedHashes) {
...
func (ewl *evictionWaitingListV2) removeFromInversedCache(rootHash []byte, hashes data.ModifiedHashes) {
	for hash := range hashes {
...
func (ewl *evictionWaitingListV2) Evict(rootHash []byte) (data.ModifiedHashes, error) {
	ewl.opMutex.Lock()
	defer ewl.opMutex.Unlock()
...
func (ewl *evictionWaitingListV2) ShouldKeepHash(hash string, identifier data.TriePruningIdentifier) (bool, error) {
	ewl.opMutex.RLock()
